...
- 1.5 秒后切换到第二阶段（坑 2 — WFP 匹配条件）：
  - 左：红色代码 `FWPM_CONDITION_IP_LOCAL_ADDRESS`，大 × 标记
  - 右：绿色代码 `FWPM_CONDITION_ALE_APP_ID`，大 ✓ 标记
- 代码部分用等宽字体 + 语法着色
...
**类型**：动画（模拟录屏）
**内容**：模拟 NetMonitor 界面的完整操作流程
**重要说明**：由于本流水线运行在 Ubuntu 上，无法实际运行 Windows 程序。因此用 React 组件模拟 NetMonitor 的界面和交互。
...
```
NetMonitor
C++17 / ETW / WFP / Win32 / GoogleTest / CMake
感谢收看
```
...
export const RemotionRoot: React.FC = () => {
  const totalDuration = Math.ceil(audioManifest.totalDurationSeconds * THEME.fps);
