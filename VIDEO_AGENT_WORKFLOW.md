...
> **本文档是 AI Agent 的执行指南，包含每个阶段的实现细节和代码模板。**
>
> **配套文档 `VIDEO_AGENT_STATE.md` 定义了 46 个独立任务、依赖关系和断点续跑机制。Agent 启动时应先读取 STATE 文档的 `pipeline-state.json`，按任务粒度跟踪执行进度。**
...
# 收集缺失的包
MISSING_PKGS=()
for cmd_pkg in "curl:curl" "wget:wget" "git:git" "make:build-essential" "jq:jq" "ffmpeg:ffmpeg" "python3:python3" "pip3:python3-pip"; do
  CMD="${cmd_pkg%%:*}"
  PKG="${cmd_pkg##*:}"
...
# Chromium 检测（可能是 chromium-browser 或 chromium）
...
if ! fc-list :lang=zh 2>/dev/null | grep -qi "noto\|wqy"; then
  MISSING_PKGS+=("fonts-noto-cjk" "fonts-noto-cjk-extra")
fi

# python3-venv 无法通过命令检测，用 dpkg 查
...
```bash
# 检测是否已安装 Node.js 20.x
if node -v 2>/dev/null | grep -q '^v20\.'; then
  echo "SKIP: Node.js $(node -v) 已安装"
else
...
```bash
# 检测虚拟环境和 edge-tts 是否已就绪
if [ -f ~/video-agent-venv/bin/activate ]; then
  source ~/video-agent-venv/bin/activate
  if command -v edge-tts &>/dev/null; then
    echo "SKIP: edge-tts 已安装 ($(edge-tts --version 2>&1 || echo 'ok'))"
  else
...
# 检测 Remotion 项目是否已初始化
if [ -f package.json ] && npx remotion --version &>/dev/null; then
  echo "SKIP: Remotion 项目已存在"
else
  npx create-video@latest . --template blank-ts --no-git
fi

# 确保额外依赖已安装（npm install 对已安装的包是幂等的）
...
Agent 使用 `VIDEO_AGENT_STATE.md` 中定义的 `pipeline-state.json` 进行任务级状态追踪。每完成一个 T0x 任务立即更新状态文件，详见 STATE 文档 §4.2。
...
`TextCard.tsx` 不应把文案写死在组件内部。统一接口建议如下：

```tsx
type TextCardVariant = 'quote' | 'closing' | 'credits';
...
素材 4 / 12 / 16 / 17 通过 `lines` 和 `variant` 传参复用同一个组件。
      ...
      {audioManifest.segments.map((audioSeg, index) => {
        ...
        const result = (
          <Fragment key={audioSeg.id}>
            ...
            {index < audioManifest.segments.length - 1 ? (
              <Sequence
                from={frameOffset + durationInFrames}
                durationInFrames={silenceGap}
                key={audioSeg.id + '-silence'}
              >
...
type BaseAssetProps = { durationInFrames: number };
type TextCardVariant = 'quote' | 'closing' | 'credits';

interface TextCardProps extends BaseAssetProps {
  lines: string[];
  variant: TextCardVariant;
}

type AssetComponentProps = BaseAssetProps | TextCardProps;

interface AssetRenderSpec {
  component: React.FC<any>;
  props: AssetComponentProps;
}

const TEXT_CARD_PRESETS: Record<4 | 12 | 16 | 17, Omit<TextCardProps, 'durationInFrames'>> = {
  4: {
    variant: 'quote',
    lines: ['花钱买一个黑盒来省钱，这逻辑说不通。'],
  },
  12: {
    variant: 'quote',
    lines: ['AI 犯错的速度很快，但改错的速度也很快。'],
  },
  16: {
    variant: 'closing',
    lines: ['不是你不需要懂技术，', '而是你可以用两个小时，走完以前两周的路。'],
  },
  17: {
    variant: 'credits',
    lines: ['NetMonitor', 'C++17 / ETW / WinDivert / Win32 / GoogleTest / CMake', '感谢收看'],
  },
};

// assetId → React 组件映射表
const ASSET_COMPONENT_MAP: Record<number, React.FC<any>> = {
  1: BillingMock,
  2: TaskManagerMock,
  3: PricingComparison,
  4: TextCard,           // 「花钱买黑盒」金句
...
// 根据 assetId 返回对应的 React 组件和 props
export function getAssetRenderSpec(id: number, durationInFrames: number): AssetRenderSpec {
  if (id === 4 || id === 12 || id === 16 || id === 17) {
    return {
      component: TextCard,
      props: {
        durationInFrames,
        ...TEXT_CARD_PRESETS[id],
      },
    };
  }

  return {
    component: ASSET_COMPONENT_MAP[id] ?? TextCard,
...
import { computeAssetTimings, getAssetRenderSpec, type Segment } from '../timing';
...
const SegmentVisuals: React.FC<{ segment: Segment; durationInFrames: number }> = ({ segment, durationInFrames }) => {
  ...
  return (
    <AbsoluteFill>
      {timings.map((t) => {
        const {component: Component, props} = getAssetRenderSpec(t.assetId, t.durationFrames);
        return (
...
# 或者渲染单帧静态图来验证
# 注意：frame=0 仍处于 2 秒淡入起点，按设计接近全黑，不适合作为验收帧
npx remotion still MainVideo --frame=75 --output=test_frame_2_5s.png
...
~/teaching-video/output/
  ...
  └── pipeline-report.txt       # 完成报告（T62 生成）
```
...
# 系统包：只安装缺失的（详见 §0.1 的检测脚本）
# 这里给出精简写法，完整检测逻辑见正文
NEED_APT=false
for cmd in curl wget git make jq ffmpeg python3 pip3; do
  command -v "$cmd" &>/dev/null || NEED_APT=true
done
command -v chromium-browser &>/dev/null || command -v chromium &>/dev/null || NEED_APT=true
if $NEED_APT; then
  sudo apt update && sudo apt install -y curl wget git build-essential jq ffmpeg chromium-browser fonts-noto-cjk fonts-noto-cjk-extra python3 python3-pip python3-venv
  fc-cache -fv
fi

# Node.js：已有 v20.x 则跳过
node -v 2>/dev/null | grep -q '^v20\.' || { curl -fsSL https://deb.nodesource.com/setup_20.x | sudo -E bash - && sudo apt install -y nodejs; }

# Python venv + edge-tts：已有则跳过
if [ ! -f ~/video-agent-venv/bin/activate ]; then python3 -m venv ~/video-agent-venv; fi
source ~/video-agent-venv/bin/activate
command -v edge-tts &>/dev/null || pip install edge-tts

# Remotion 项目：已有则跳过 create，npm install 幂等可重跑
mkdir -p ~/teaching-video && cd ~/teaching-video
[ -f package.json ] || npx create-video@latest . --template blank-ts --no-git
npm install shiki gray-matter && npm install -D ts-node typescript @types/node
...
# 验证编译通过
npx remotion studio  # 或 npx remotion still MainVideo --frame=75 --output=test_frame.png
