...
>
> **配套文档 `VIDEO_AGENT_WORKFLOW.md` 包含每个任务的实现细节、代码模板和错误恢复策略。Agent 在执行某个任务时，应查阅 WORKFLOW 文档中对应章节获取实现指导。**
...
| `running` | 正在执行 | 如果 Agent 重启时发现此状态，先重置为 `pending`，再重新调度 |
| `completed` | 成功完成 | 跳过，不重复执行 |
| `error` | 执行失败 | 检查 `retries` 次数，未超限则重置为 `pending` 重试；超限则保留 `error`，阻塞其依赖链 |
...
2. 启动时先做一次状态归一化：
  a. status == running → 视为中断，设为 pending，并写回状态文件
  b. status == error 且 retries < 3 → 设为 pending，并写回状态文件

3. 按任务 ID 排序，循环查找可执行任务：
  a. status == completed 或 skipped → 跳过
  b. status == error 且 retries >= 3 → 标记为终态错误，不再自动重试
  c. status == pending → 检查 depends_on 中所有前置任务是否 completed/skipped
    - 是：加入 ready 队列执行
    - 否：本轮跳过，等待其他分支完成后再次扫描

4. 任务执行前：设 status = running，写入状态文件
5. 任务执行后：设 status = completed（或 error），写入状态文件
6. 没有 ready 任务时：
  - 若仍存在 status == error 且 retries >= 3 的任务：报告阻塞链
  - 若全部任务均为 completed/skipped：输出总结报告
  - 否则继续等待并重新扫描
```
...
| `T01_apt_install` | 安装系统包 | T00 | `which ffmpeg && which chromium-browser && which jq && fc-list :lang=zh` | ffmpeg, chromium, jq, 字体 |
...
### STAGE 2：语音合成（T20 ~ T23）—— 与 STAGE 3 可并行
...
| `T21_silence` | 生成静音文件 | T01 | `test -f public/audio/silence.mp3` | silence.mp3 |
...
| `T42_video_root` | 创建 Root.tsx + Video.tsx + index.ts | T21, T41, T22, T23 | `test -f src/Root.tsx && test -f src/Video.tsx` | Root.tsx, Video.tsx, index.ts |
| `T43_compile_check` | TypeScript 编译验证 + 单帧渲染测试 | T42 | `npx remotion still MainVideo --frame=75 --output=test_frame.png` 成功 | test_frame.png |
...
Agent 执行某个任务时，根据此表定位到 `VIDEO_AGENT_WORKFLOW.md` 中的实现细节：

| 任务 ID 范围 | WORKFLOW 章节 |
|-------------|--------------|
| T00 ~ T06 | STAGE 0（§0.0 ~ §0.7） |
| T10, T11 | STAGE 1（§1.1 ~ §1.6） |
| T20_tts_* | STAGE 2（§2.2 执行流程） |
| T21_silence | STAGE 2（§2.5 静音间隔） |
| T22_audio_manifest | STAGE 2（§2.4 音频清单） |
| T23_vtt_to_json | STAGE 2（§2.5.1 VTT 预解析） |
| T30_theme | STAGE 3（§3.2 全局视觉规范） |
| T30_code_highlight | STAGE 3（§3.3 素材 8 的前置脚本） |
| T31_TextCard | STAGE 3（§3.3 素材 4 / 12 / 16 / 17） |
| T31_BillingMock | STAGE 3（§3.3 素材 1） |
| T31_TaskManagerMock | STAGE 3（§3.3 素材 2） |
| T31_PricingComparison | STAGE 3（§3.3 素材 3） |
| T31_ChatSimulation | STAGE 3（§3.3 素材 5） |
| T31_ArchitectureDiagram | STAGE 3（§3.3 素材 6） |
| T31_TokenBucket | STAGE 3（§3.3 素材 7） |
| T31_CodeGeneration | STAGE 3（§3.3 素材 8） |
| T31_TestResults | STAGE 3（§3.3 素材 9） |
| T31_ModuleTimeline | STAGE 3（§3.3 素材 10） |
| T31_BugComparison | STAGE 3（§3.3 素材 11） |
| T31_NetMonitorDemo | STAGE 3（§3.3 素材 13） |
| T31_TimeComparison | STAGE 3（§3.3 素材 14） |
| T31_AIInternAnalogy | STAGE 3（§3.3 素材 15） |
| T31_SubtitleOverlay | STAGE 3（§3.4 字幕覆盖层） |
| T40_timing | STAGE 4（§4.3 段落内素材分配时序） |
| T41_segment_visuals | STAGE 4（§4.4 SegmentVisuals 组件） |
| T42_video_root | STAGE 4（§4.1 + §4.2 Root.tsx / Video.tsx） |
| T43_compile_check | STAGE 5（§5.1 预览验证） |
| T50_preview_frames | STAGE 5（§5.1 渲染关键帧） |
| T51_full_render | STAGE 5（§5.2 完整渲染） |
| T60_normalize | STAGE 6（§6.1 音频标准化） |
| T61_quality_check | STAGE 6（§6.2 质量校验） |
| T62_summary | STAGE 6（§6.4 最终输出） |
...
**并行安全要求**：所有读写 `pipeline-state.json` 的脚本都必须在同一个锁文件上串行化，禁止多个并行任务直接读改写同一个 JSON 文件。
...
LOCK_FILE=~/teaching-video/pipeline-state.lock
...
TASKS=(
  ...
  "T42_video_root:T21_silence,T41_segment_visuals,T22_audio_manifest,T23_vtt_to_json"
...
# 用 flock + node 构建 JSON（避免并行初始化和 bash JSON 转义问题）
(
flock -x 9
...
```bash
#!/bin/bash
# next-tasks.sh — 先归一化中断/可重试状态，再输出当前所有可执行任务
...
for (const [id, task] of Object.entries(state.tasks)) {
  if (task.status === 'running') {
    task.status = 'pending';
    task.note = task.note ? task.note + ' | recovered from interrupted run' : 'recovered from interrupted run';
    mutated = true;
  }
...
if (ready.length === 0) {
  const terminalErrors = Object.entries(state.tasks).filter(([,t]) => t.status === 'error' && (t.retries || 0) >= 3);
  const unfinished = Object.entries(state.tasks).filter(([,t]) => !['completed', 'skipped'].includes(t.status));
  if (terminalErrors.length > 0) {
    console.log('BLOCKED by terminal errors:', terminalErrors.map(([id, t]) => id + '(' + t.note + ', retries=' + t.retries + ')').join(', '));
  } else if (unfinished.length === 0) {
    console.log('ALL DONE');
  } else {
    console.log('WAITING: no ready tasks yet');
  }
} else {
...
```json
{
  ...
  "tasks": {
    ...
    "T21_silence":           { "status": "pending",   "ts": "", "note": "", "retries": 0, "depends_on": ["T01_apt_install"] },
    ...
    "T42_video_root":        { "status": "pending",   "ts": "", "note": "", "retries": 0, "depends_on": ["T21_silence","T41_segment_visuals","T22_audio_manifest","T23_vtt_to_json"] },
